...
export const CartItems: FC = React.memo(() => {
  ...
  return (
    <Box className="py-3 px-4 pb-24 space-y-4 min-h-fit">
      {cart.length > 0 ? (
        <ProductPicker product={editingItem?.product} selected={editingItem}>
          {({ open }) => (
            <ListRenderer
              ...
              limit={5}
              onClick={() => {}}
              itemClassName="bg-white rounded-lg shadow-sm border border-gray-200 p-4 mb-2 min-h-[80px] hover:bg-gray-50 hover:shadow-md transition-all duration-200"
                ...
                <Box flex className="flex-1 justify-between items-start">
                  ...
                  <Box flex className="items-center space-x-4">
                    <Box flex className="items-center space-x-2">
                      <Button
                        size="small"
                        className="w-8 h-8 min-w-0 rounded-md p-0 flex items-center justify-center bg-gray-100 text-gray-800 border border-gray-300 hover:bg-gray-200"
                        onClick={(e) => {
                          e.stopPropagation();
        ...
        <Box className="text-center">
          <Text
            className="bg-white border border-gray-200 rounded-lg shadow-sm py-8 px-4 text-gray-500"
            size="normal"
          >
            Không có sản phẩm trong giỏ hàng
          </Text>
          <Button
            className="mt-4 bg-primary text-white rounded-md hover:bg-primary-dark px-6 py-2"
            onClick={() => {
