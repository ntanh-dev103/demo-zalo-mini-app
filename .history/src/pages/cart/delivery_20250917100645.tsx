...
export const Delivery: FC = React.memo(() => {
  ...
  const debouncedSetNote = debounce(setNote, 300);
    ...
    <Suspense fallback={<Text>Loading...</Text>}>
      <Box className="space-y-3 px-4 pt-4 pb-20 border-t border-gray-200">
              ...
              right: (
                <Box flex>
                  <ElasticTextarea
                    ...
                    maxRows={4}
                    value={note}
                    onChange={(e) => debouncedSetNote(e.currentTarget.value)}
                  />
                </Box>
              ),
            },
        ...
        <Box className="p-4 space-y-2 border-t">
          <Text>Thành tiền: {formatCurrency(totals.subtotal)}</Text>
          {coupon && (
          ...
          <Text>Phí vận chuyển: {formatCurrency(totals.shippingFee)}</Text>
          <Text className="font-bold">
            Tổng thanh toán: {formatCurrency(totals.total)}
          </Text>
        </Box>
      </Box>
    </Suspense>
