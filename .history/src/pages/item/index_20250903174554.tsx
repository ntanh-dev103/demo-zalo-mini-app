  ...
  return (
    <Box className="p-4 max-w-4xl mx-auto">
      <Text className="text-3xl font-bold mb-2">
        Chi Tiết Sản Phẩm: <span className="text-primary">Thông Tin Toàn Diện</span>
      </Text>
      <Box className="flex flex-col md:flex-row gap-6">
        {/* Image Gallery */}
        <Box>
          <img src={selectedImage} alt={item.name} className="w-96 h-72 object-cover rounded-xl mb-2" />
          <Box className="flex gap-2">
            {item.images.map((img, idx) => (
              <img
                ...
                src={img}
                alt={`Thumb${idx + 1}`}
                className={`w-16 h-16 object-cover rounded-lg border-2 ${selectedImage === img ? "border-primary" : "border-transparent"}`}
        ...
        <Box className="flex-1">
          <Box className="flex items-center gap-2 mb-2">
            ...
            {item.isNew && (
              <span className="bg-green-100 text-green-600 px-3 py-1 rounded-full text-xs font-semibold">Mới nhất</span>
            )}
          </Box>
                ...
                <Box key={opt.label} className="flex items-center gap-2">
                  ...
                  {opt.values.map((val) => (
                    <Button
                      key={val}
                      size="small"
                      className={`rounded-full px-3 ${selectedOptions[opt.label] === val ? "bg-primary text-white" : "bg-gray-100"}`}
                      onClick={() => setSelectedOptions({ ...selectedOptions, [opt.label]: val })}
                    >
                      {val}
                    </Button>
                  ))}
      ...
      <Box className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-6">
        ...
        <Box className="col-span-1 bg-background p-4 rounded-xl">
          <Text className="font-bold mb-2">Tầm quan trọng của trang sản phẩm</Text>
          <Text>Trang chi tiết sản phẩm rõ ràng, hấp dẫn trực tiếp thúc đẩy quyết định mua hàng và tăng tỷ lệ chuyển đổi.</Text>
        </Box>
      </Box>
    </Box>
  );
