...
const NotificationList: FC = () => {
  ...
  return (
    ...
    items={notifications}
    renderLeft={(item) => (
      <img className="w-10 h-10 rounded-full hover:shadow-blue-500/70 hover:scale-110" src={item.image} />
    )}
    renderRight={(item) => (
      <Box
        key={item.id}
        className="p-3 rounded-lg shadow-md bg-white hover:shadow-blue-500/70 hover:scale-110 transition"
      >
