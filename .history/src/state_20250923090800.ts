...
export const totalPriceWithCouponState = selector<number>({
  ...
  get: ({ get }) => {
    ...
    if (!coupon) /* Line 159 omitted */

    if (coupon.discountType === "percent") {/* Lines 162-163 omitted */}

    if (coupon.discountType === "fixed") {/* Lines 166-167 omitted */}
...
export const finalTotalState = selector({
  ...
  get: ({ get }) => {
    ...
    // discount
    let discount = 0;
    if (coupon) {/* Lines 187-192 omitted */}
...
>>>>>>> a7996cb9e191283f50d9e47a9b2bde7083e6e9c7
