...
interface ListRendererProps<T> {
  ...
  renderKey: (item: T) => string; // required now
  onClick?: (item: T) => void;
  noDivider?: boolean;
}
...
export function ListRenderer<T>({
  ...
  const visibleItems = isCollapsed ? collapsedItems : items;
          ...
          >
            ...
            <Box className="flex-1 min-w-0 relative">
              ...
              {!noDivider && i < list.length - 1 && (
                <hr className="absolute left-0 -right-4 -bottom-4 border-divider border-t-[0.5px]" />
              )}
            </Box>
          </div>
        ))}
