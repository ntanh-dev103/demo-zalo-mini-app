...
export function calcFinalPrice(product: Product, options?: SelectedOptions) {
  ...
  if (product.sale) {
    if (product.sale.type === "fixed") {
      ...
      finalPrice = product.price * (1 - product.sale.percent*0.01);
    }
  }
    ...
    finalPrice = selectedOptions.reduce((price, option) => {
      if (option.priceChange) {
        if (option.priceChange.type == "fixed") {
          ...
          return price + product.price * option.priceChange.percent*0.01;
        }
      }
      return price;
    }, finalPrice);
...
const pay = (amount: number, description?: string) =>
    createOrder({
      ...
      item: [],
      amount: amount,
      mac: generateMac(amount, description), // Add mac property
      success: (data) => {
  ...
  function generateMac(amount: number, description?: string): string {
    // Replace this with your actual MAC generation logic
    return `${amount}-${description || "default"}`;
  }
